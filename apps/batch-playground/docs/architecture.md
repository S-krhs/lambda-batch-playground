...
| 層 | 置くもの | 置かないもの |
| --- | --- | --- |
| `src/handlers/<handler>/handler.ts` | Lambda エントリポイント、担当 job への委譲 | ジョブ選択条件、業務ロジック、外部連携詳細 |
| `src/handlers/<handler>/routes.ts` | イベントの `job` 名や interaction の custom_id prefix から、担当 job・feature への解決 | 各ジョブの処理内容、メッセージ生成、外部 API 詳細 |
| `src/handlers/<handler>/jobs/` | イベント値の正規化、feature 呼び出し、integration 呼び出し、共通レスポンス作成 | ルーティング判定、内部処理の詳細、外部 API 詳細 |
...
| `src/handlers/<handler>/jobs/runtime-settings/` | job ごとの実行時設定の型と、SST link / 環境変数からの解決 | 個別 feature の値、event/response の型・契約、外部サービスとの通信 |
...
```text
handlers/batch:        handler -> routes -> jobs -> features
...
- Lambda の起動イベント検証 schema と応答型は `src/handlers/<handler>/schemas/` に置く。起動イベントは `unknown` として受け取り、routes で使う直前に schema で検証・正規化する。
- 実行時設定（linked secret / 環境変数）の型と解決は `src/handlers/<handler>/jobs/runtime-settings/<job>-setting-resolver.ts` に置く。
