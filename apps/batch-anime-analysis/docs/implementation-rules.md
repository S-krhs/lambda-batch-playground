...
- SQS 送信は integration package `@lambda-batch-playground/integration-sqs` に委譲し、queue URL 解決は job ごとの設定解決として `src/jobs/runtime-settings/orchestrator-setting-resolver.ts` に置く。
