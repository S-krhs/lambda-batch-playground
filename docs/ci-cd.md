...
`npm run deploy` は `sst deploy --stage production` を実行します。
...
この role には、SST が production stage のリソースを作成・更新するための権限が必要です。
...
- `npx sst diff --stage production` で意図しない差分が出ていないか。
