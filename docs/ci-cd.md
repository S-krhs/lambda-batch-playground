...
8. `npm run deploy`
9. `npm run discord:sync`（宣言済みスラッシュコマンドを Discord guild へ同期。deploy 後に実行する）
...
- `DISCORD_APPLICATION_ID`（スラッシュコマンド同期先の Discord application ID。`npm run discord:sync` step だけが使う）
- `DISCORD_GUILD_ID`（スラッシュコマンド同期先の Discord guild ID。`npm run discord:sync` step だけが使う）

`DISCORD_BOT_TOKEN` は SST secret（`SST_SECRET_DiscordBotToken`）と `npm run discord:sync` の両方で使うため、既存の同名 Secret を共用します。
...
スラッシュコマンドの定義は [apps/batch-playground](../apps/batch-playground) の `command-definitions.ts` で宣言し、CD の deploy 後に `npm run discord:sync` で Discord guild へ `PUT .../commands`（bulk overwrite）します。宣言に無いコマンドは Discord 側から削除されるため、コードと登録コマンドが常に一致します。冪等なので毎 deploy 実行して問題ありません。
...
- GitHub Secrets に `DISCORD_BOT_TOKEN`・`DISCORD_APPLICATION_ID`・`DISCORD_GUILD_ID` を登録する。`DISCORD_BOT_TOKEN` は SST secret と同値。
- SST secret も設定する: `npx sst secret set DiscordBotToken <token>` / `npx sst secret set DiscordInteractionPublicKey <public-key>`。
